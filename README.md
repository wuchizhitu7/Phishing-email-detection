...
<img width="1448" height="1086" alt="流程图" src="https://github.com/user-attachments/assets/ab99591c-a9cd-4cc1-8aed-33734540d716" />
...
│                        🎯 AI 钓鱼邮件检测                           │
├─────────┬───────────────────────────────────────────────────────────┤
│         │   AI 文本 token 序列平滑可预测，BERT MLM 计算困惑度        |
│  第1层  │   显著低于人类写作。取 log 归一化后作为第 9 维数值特征，   |
│ 统计层  │   同时送入 RF 和 BERT 的融合层，为模型提供"AI感"量化参考。 │
│         │             关键特征：perplexity                          │
├─────────┼───────────────────────────────────────────────────────────┤
│         │   训练集含 5,000 封 LLM 生成钓鱼邮件，覆盖账号安全、        │
│  第2层  │   福利补贴、快递物流、企业冒名、退款理赔 5 大类型，          │
│ 语义层  │   每种含紧急胁迫 / 温和商务两种语气。BERT 解冻顶层 4 层，    │
│         │   学习 AI 文本特有话术组合与篇章结构。                       │
│         │             关键模型：bert-base-chinese (微调)              │
├─────────┼───────────────────────────────────────────────────────────┤
│         │   AI 生成邮件的链接同样为伪造域名：仿冒品牌名 + 连字符       │
│  第3层  │   + 钓鱼敏感词。URL 安全模块对链接进行 10 项本地规则评级，   │
│ 链接层  │   高风险链接通过后处理加权 (+0.08~0.15) 修正最终概率。       │
│         │             关键模块：url_security.py                       │
└─────────┴───────────────────────────────────────────────────────────┘
```
